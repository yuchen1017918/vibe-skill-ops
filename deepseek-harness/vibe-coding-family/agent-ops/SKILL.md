---
name: agent-ops
description: 多Agent运行全栈（v2.0 合并自 agent-loop）：工位工作区（每人专属 MEMORY/PLAN/EXPERIENCE）+ 平级协作协议（协作对+三步闭环+证据验证） + 循环指挥（汇报格式+状态决策表+无人自动开发+心跳检测+信任链）。 当启用 dev-team / 多Agent协作 / 子Agent需要配合或指挥时加载。 触发词：工位、工作区、多Agent协作、协作协议、自动开发、无人值守、循环指挥、心跳。
