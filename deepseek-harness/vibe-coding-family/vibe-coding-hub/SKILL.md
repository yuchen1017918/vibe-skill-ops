...
description: Vibe-Coding Skill 全家桶 L1 总目录 — 四层组织路由入口。 当用户提到 vibe coding、快速开发 MVP、迭代式写代码、skill 全家桶、 或需要"这个开发任务该用哪个 skill"时，先加载本 skill。 四层路由：L1 组织形态(团队/企业) → L2 开发模式(助手/全盘) → L3 四格场景 → L4 执行 skill。
family-type: tool
...
| 类型 | 含义 | 调用方式 | 自建 L3 示例 |
|------|------|----------|--------------|
| `tool` | 做一件具体事，有明确输入输出 | 按需显式调用 | snapshot-notes / security-audit / project-init / china-env-adapt |
| `workflow` | 管一段流程，有阶段和状态 | 阶段推进式调用 | project-init / agent-ops / snapshot-notes |
...
| 级别 | 处理方式 |
|------|----------|
| **L0 微小改动** | 跳过 project-init，直接 tool 级快进快出，不上全套 |
| **L1 中等功能** | 三件事：明确边界 → 步骤追踪 → 完成前验证 |
| **L2 高风险/核心链路** | 完整流程：dev-team + agent-ops + project-init，买确定性 |
...
- 判定 L0/L1 → 不加载 project-init / agent-ops 全套
...
| 国内环境适配 | `china-env-adapt` + `project-init` | "换源" / "国内环境" |
...
```
路由失败/加载异常 → fallback-general-dev 三步基本盘;报错≥3次→vibe-terminal-safe 只读模式;
...
2. workflow > tool（"计划"优先 project-init，而非拆分前的脚手架）
3. 最近使用 > 历史记录
...
> **机制说明移 references**（v1.4 渐进披露瘦身，按需加载）：执行走样日志 / 确认交互契约 / 新手梯度 / 兼容性声明 / 来源说明 → `references/route-mechanisms.md`；addyosmani 精华补充 → `references/addyosmani-essence.md`。需要对应机制时再加载。
...
> v1.3 大道至简：契约层压缩至 ≤5 行，36 skill × 5 行 ≈ 155 行常驻（较原 310 行减半）。
