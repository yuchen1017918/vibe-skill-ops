...
> 目标：把 vibe-coding skill 全家桶（**28 个 skill**）安装到目标 Agent 的 skill 目录。
>
...
- **规模**：6 个目录 skill + 22 个 L3 skill = 28 个；另附 1 个终端硬拦截脚本（vibe-terminal-safe/scripts/harden-shell.sh）

> 安装后用户使用：发需求 → Agent 按触发词加载 hub → 三层路由 → 落到具体 skill 流程。
...
# 2. 复制全家桶到 skills 目录（注意是 vibe-coding-family 子目录）
mkdir -p ~/.hermes/skills
cp -r /tmp/vibe-skill-ops/vibe-coding-family ~/.hermes/skills/

# 3. 验证
ls ~/.hermes/skills/vibe-coding-family/                          # 应有 28 个 skill 目录
find ~/.hermes/skills/vibe-coding-family -name SKILL.md | wc -l   # 应为 28
```
...
2. 向用户说明：**「全家桶安装完成，共 28 个 skill，可以开始 vibe coding 了」**
