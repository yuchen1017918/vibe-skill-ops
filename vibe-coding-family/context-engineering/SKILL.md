...
description: |
  上下文工程：5 层上下文层级（规则文件→规格→源码→错误输出→会话历史）+
  文件信任分级（Trusted/Verify/Untrusted）+ 预任务上下文加载策略。
  大项目 token 优化核心——按层级喂上下文，不全文塞。
  当大项目启动、token 超预算、或"上下文不够用/太乱"时加载。
  触发词：上下文、context、token优化、喂上下文、信任分级。
version: 1.1.0
author: Hermes Agent
license: MIT
metadata:
  hermes:
    type: tool
    tags: [vibe-coding, context, token, project-init, optimization]
    related_skills: [project-init, snapshot-notes, vibe-code-search, dev-assistant]
---
...
## 4️⃣ 会话内压缩技术（v1.1 新增 — claudekit context-engineering 精华）

本地版管「喂什么」；这节管「喂进去之后怎么省」。三条技术按场景选：

| 技术 | 是什么 | 适用场景 |
|------|--------|----------|
| **Compaction 压缩** | 把冗长中间产物（长日志/大 diff/旧对话）替换成一句话摘要，原文落盘可查 | 长会话中段、工具输出过大时 |
| **Masking 掩蔽** | 喂模型前把无关/敏感部分打码（`<redacted 37 lines>`），保留结构 | 大文件只关心其中几段时 |
| **Caching 缓存** | 稳定不变的内容（规则文件/锁文件/官方规范）标记为缓存态，跨轮次不重读 | L1 Trusted 文件、常驻指令 |

**原则**：压缩保结论丢细节、掩蔽保结构丢内容、缓存保稳定丢新鲜度——三者都留"原文路径"可回溯，压缩≠丢弃。

## 5️⃣ 预任务上下文加载清单（任务开始时 30 秒）
