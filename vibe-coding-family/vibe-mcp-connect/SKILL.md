...
description: |
  官方 filesystem/git MCP 接入 Hermes 的配置说明。当需要 MCP 文件操作、
  MCP 版本控制、或想让 Agent 通过 MCP 获得隔离的文件/git 能力时加载。
  附官方 README 参考（references/）。
version: 1.0.0
author: Hermes Agent
license: MIT
metadata:
  hermes:
    type: tool
    tags: [vibe-coding, mcp, filesystem, git, configuration]
    related_skills: [vibe-coding-hub, hermes-mcp-setup, native-mcp]
---
