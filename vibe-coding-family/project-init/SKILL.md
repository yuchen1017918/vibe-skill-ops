...
name: project-init
description: |
  项目初始化与规划 skill（v1.3 合并自 project-scaffold + plan-workflow）：
  一键建骨架（标准目录+依赖+git+快照+宪法）+ 技术栈背景嗅探 + 任务拆分计划。
  当用户说"新建项目"、"初始化项目"、"做个计划"、"拆分任务"、
  "规划一下"、"脚手架"时加载。
  触发词：新建项目、初始化、脚手架、做计划、拆分任务、规划。
version: 1.0.0
author: Hermes Agent
license: MIT
metadata:
  hermes:
    type: workflow
    tags: [vibe-coding, init, scaffold, plan, stack, project]
    related_skills: [vibe-coding-hub, snapshot-notes, git-workflow, dev-core-hub, requirement-clarify, china-env-adapt]
---
